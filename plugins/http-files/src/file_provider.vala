...
public class FileProvider : Dino.FileProvider, Object {
	...
	private static Regex[] user_permitted_url_regexes = {/https?:\/\/fars\.ee\/([^\s#]*)/,
		/https?:\/\/img\.vim-cn\.com\/([^\s]*)/,
	};
    ...
    private class ReceivedMessageListener : MessageListener {
        ...
        public override async bool run(Entities.Message message, Xmpp.MessageStanza stanza, Conversation conversation) {
            ...
            if (normal_file || omemo_file) {
				...
				foreach (var user_permitted_url_regex in user_permitted_url_regexes) {
					if(user_permitted_url_regex.match(message.body, 0, out match_info)) {
						int start, end;
